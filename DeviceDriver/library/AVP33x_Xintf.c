...
//---------------------------------------------------------------------------
// 外部总线接口模块初始化函数
// XINTF的时钟配置代码不能放在XINTF中，否则会出现意料不到的问题。
void InitXintf(void)
{
    // 以下配置都是与复位后寄存器的默认状态一致，用户可根据自己的系统结构配置相应的参数
    ...
    EALLOW;
    XintfRegs.XINTCNF2.bit.XTIMCLK = 1;     // XINTF模块的时钟为XTIMCLK = SYSCLKOUT/2
    XintfRegs.XINTCNF2.bit.WRBUFF = 0;      // 写缓冲深度为0
    XintfRegs.XINTCNF2.bit.CLKOFF = 0;      // 使能XCLKOUT时钟输出
    XintfRegs.XINTCNF2.bit.CLKMODE = 1;     // 外部总线时钟输出XCLKOUT = XTIMCLK/2
