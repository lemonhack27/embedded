/******************************************************************************
文件名                     : AVP33x_SysCtrl.h
作者                         : 湖南进芯电子科技有限公司
功能描述                 : AVP33x芯片系统控制寄存器定义
创建时间                 : 06-25-2019
版本                         : V1.0
修订时间                 :
修订内容                 :
...
#ifndef AVP33x_SYS_CTRL_H
#define AVP33x_SYS_CTRL_H
...
//----------------------------------------------------
// 高速外设时钟寄存器（HISPCP）位定义:
struct HISPCP_BITS  {   // 位描述
...
//----------------------------------------------------
// 低速外设时钟寄存器（LOSPCP）位定义:
struct LOSPCP_BITS  {   // 位描述
...
//----------------------------------------------------
// 外设时钟控制寄存器1（PCLKCR1）位定义:
struct PCLKCR1_BITS  {    // 位描述
...
//----------------------------------------------------
// 低功耗模式0控制寄存器（LPMCR0）位定义:
struct LPMCR0_BITS {     // 位描述
...
//----------------------------------------------------
// 内核电压配置控制寄存器（VDD_CORE）位定义：
struct VDD_CORE_BITS {   // 位描述
    Uint16 VDDCORE:3;  	 // 0     内核电压配置位
    Uint16 rsvd1:13;     // 15:3  reserved
};

union VDD_CORE_REG {
    Uint16 all;
    struct VDD_CORE_BITS bit;
};
/******************* 内核电压配置说明 *******************
...
struct SYS_CTRL_REGS {
	...
	Uint16              rsvd2[4];  // 2-5   
	union   VDD_CORE_REG  VDDCORE; // 6:    0x7016   
	Uint16	   	 		rsvd8[3];  // 7-9:
...
#endif  // end of AVP33x_SYS_CTRL_H definition
